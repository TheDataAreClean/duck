...
- **Walk to a screen edge** to move to the next room (fade transition)
...
└── render.js   ← canvas.js, constants.js, state.js, duck.js,
                   world.js, data.js, particles.js, dpad.js, ui.js
```
...
### `constants.js`
Single source of truth for every tunable value and the colour palette. Edit here to change game feel.
...
### `ui.js`
Info badge and info card overlay.

**Info badge** — pulsing `[i]` button at top-left, visible only when duck is within `LANDMARK_DIST` of a landmark. Pulse uses `Math.sin(frame * 0.12)`.
...
### `dpad.js`
D-pad centred at game coordinate (64, 125). `hitDpad(gx, gy)` uses a single proximity + quadrant test — returns `'up' | 'down' | 'left' | 'right' | null`. `drawDpad()` renders four 8-bit buttons with active highlights.
...
6. Room name (top) + "cubbon park" (bottom) at 30% opacity
...
3. No other changes needed — `prerenderAllRooms()` picks it up automatically
